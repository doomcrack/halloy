//! Add-member dialog: collects a peer address to invite into a group
//! (QML `AddMemberDialog` parity). Return submits, the entry is trimmed,
//! and the draft persists across a dismissal — it clears only once the
//! invite is sent.
...
impl AddMember {
    pub fn new(convo_id: ConvoId, draft: &str) -> Self {
        Self {
            convo_id,
    ...
    pub fn view<'a>(&'a self, theme: &'a Theme) -> Element<'a, ModalMessage> {
        let address_field = text_editor(&self.content)
            ...
            .on_action(|action| ModalMessage::AddMember(Message::Edit(action)))
            .key_binding(|key_press| {
                ...
                if matches!(
                    ...
                    Some(text_editor::Binding::Custom(ModalMessage::AddMember(
                        Message::Submit,
                    )))
                } else {
        ...
        container(
            column![
                ...
                text("Paste the address of the person to invite:")
                ...
                row![
                    ...
                    action_button(
                        "Add",
                        can_submit.then_some(ModalMessage::AddMember(
                            Message::Submit
                        )),
                    ),
                ],
